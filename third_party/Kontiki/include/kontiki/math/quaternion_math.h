...
Eigen::Quaternion<T> expq(const Eigen::Quaternion<T>& q)
{
  ...
  auto v_squared = q.vec().squaredNorm(); // 平方
...
Eigen::Matrix<T, 3, 1> angular_velocity(const Eigen::Quaternion<T> &q, const Eigen::Quaternion<T> &dq) {
  ...
  return w.vec(); // a vector expression of the imaginary part (x,y,z)
};

template<typename T>
Eigen::Quaternion<T> embed_vector(const Eigen::Matrix<T, 3, 1> &v) {
  return Eigen::Quaternion<T>(T(0), v(0), v(1), v(2)); //实部在前
}

// 全局系下的角速度和四元数求解dq dq=0.5w*q
template<typename T>
